...
def save_response_content(response, destination,nb):
    CHUNK_SIZE = 32768
    print(destination)
    fname = re.search(r'filename\=\"(.*)\"', response.headers['Content-Disposition']).group(1) 
...
class Application(tk.Frame):
    ...
    def askopenfile(self):
        ...
        response = requests.get('https://docs.google.com/spreadsheet/ccc?key=1bt74MVjicNVHH0QEhCSgvXacARq1o36oEDnc-Y8Zzv4&output=csv')
        ...
        nb = 0;
        for row in reader:
              self.var.set("Téléchargement de l'image "+str(nb)+"       Label : "+row[1])
              chemin = destination+'/'+row[1]
              os.makedirs(chemin, exist_ok=True) 
              download_file_from_google_drive(row[2][-33:], chemin,nb)
              self.p.step()    
              self.p.update() 
              nb+=1
